...
const ACTION_LABELS = {CONTACT_LEAD:"Contactar lead",GET_VIN:"Obtener VIN",REVIEW_VIN:"Revisar VIN",COMPLETE_LOGISTICS:"Completar logÃ­stica",PREPARE_QUOTE:"Preparar cotizaciÃ³n",FOLLOW_UP_PROPOSAL:"Dar seguimiento a propuesta",RESOLVE_OBJECTION:"Resolver objeciÃ³n",FORMALIZE_SERVICE:"Formalizar servicio",HANDOFF_OPERATIONS:"Entregar a Operaciones",COLLECT_FREIGHT:"Cobrar flete"};
function vehicle(l){return [l.vehicle_year,l.vehicle_make,l.vehicle_model,l.vehicle_trim].filter(Boolean).join(" ")||l.vehicle_label||"VehÃ­culo por definir"}
...
function CrmCommercialPage({supabase,userId,userName="Usuario E&R",onOpenQuote,onOpenLinkedQuote,onSendLinkedQuoteWhatsApp,refreshSignal=0}){
 ...
 const automaticFollowUpText=nextNoResponsePlan.exhausted
  ?"Cadencia DÃ­a 1 / 3 / 7 / 14 completada. Requiere decisiÃ³n de nutriciÃ³n o cierre."
  :"Si no responde: prÃ³ximo recordatorio automÃ¡tico DÃ­a "+nextNoResponsePlan.day+". Si estÃ¡ interesado: seguimiento automÃ¡tico en 2 dÃ­as.";
 const load=useCallback(async()=>{setLoading(true);setError("");try{
  const {data:orgId,error:orgError}=await supabase.rpc("current_organization_id_v3821");if(orgError)throw orgError;
  const [s,l,t,c,a,q]=await Promise.all([
  ...
  const e=[s.error,l.error,t.error,c.error,a.error,q.error].find(Boolean);if(e)throw e;
  setStages(s.data||[]);setLeads(l.data||[]);setTasks(t.data||[]);setContacts(c.data||[]);setAdvisors(a.data||[]);setQuoteTotals(q.data||[]);
 ...
 async function assignAdvisor(leadId,advisorId){setError("");setNotice("");try{const value=advisorId||null;const {error:e}=await supabase.from("crm_leads").update({assigned_to:value,updated_by:userId||null}).eq("id",leadId);if(e)throw e;const lead=leads.find(x=>x.id===leadId);if(lead)await supabase.from("crm_activities").insert({organization_id:lead.organization_id,lead_id:lead.id,contact_id:lead.contact_id,activity_type:"ADVISOR_ASSIGNED",title:"Responsable comercial actualizado",description:value?`Asignado a ${advisorById[value]?.full_name||advisorById[value]?.email||"asesor"}`:"Lead sin asesor asignado",metadata:{assigned_to:value},created_by:userId||null});setNotice("Responsable comercial actualizado.");await load();if(selected?.id===leadId)setSelected(x=>x?{...x,assigned_to:value}:x)}catch(e){setError(e?.message||"No fue posible asignar el asesor.")}}
 async function createLead(e){e.preventDefault();setSaving(true);setError("");setNotice("");try{const {data:orgId,error:oe}=await supabase.rpc("current_organization_id_v3821");if(oe)throw oe;if(!orgId)throw new Error("No se pudo determinar la organizaciÃ³n activa.");const {data:c,error:ce}=await supabase.from("crm_contacts").insert({organization_id:orgId,full_name:String(form.full_name ?? "").trim(),phone:String(form.phone ?? "").trim()||null,whatsapp_phone:String(form.phone ?? "").trim()||null,email:String(form.email ?? "").trim()||null,source:"CRM",created_by:userId||null,updated_by:userId||null}).select().single();if(ce)throw ce;const {data:l,error:le}=await supabase.from("crm_leads").insert({organization_id:orgId,contact_id:c.id,assigned_to:userId||null,vin:String(form.vin ?? "").trim()||null,vehicle_label:String(form.vehicle_label ?? "").trim()||null,priority:form.priority,created_by:userId||null,updated_by:userId||null}).select().single();if(le)throw le;const {error:me}=await supabase.rpc("crm_move_lead_stage_v3990",{p_lead_id:l.id,p_stage_code:"NEW_LEAD",p_note:"Lead creado desde CRM Comercial"});if(me)throw me;setForm({full_name:"",phone:"",email:"",vin:"",vehicle_label:"",priority:"NORMAL"});setShowNew(false);setNotice("Lead creado y tarea inicial generada correctamente.");await load()}catch(e){setError(e?.message||"No fue posible crear el lead.")}finally{setSaving(false)}}
 ...
 if(Number(stages.find(x=>x.code===lead.stage_code)?.position)===8){
  ...
  // Mismo criterio oficial de Usuarios Internos: misma organizaciÃ³n + ADMIN/OPERADOR + activos.
 ...
 async function identifyVin(){const vin=String(edit.vin||"").trim().toUpperCase().replace(/\s+/g,"");if(vin.length!==17){setError("El VIN debe contener 17 caracteres.");return}setVinLoading(true);setError("");try{const {data,error:e}=await supabase.functions.invoke("identify-vin",{body:{vin}});if(e)throw e;if(!data?.success)throw new Error(data?.error||"No fue posible identificar el vehÃ­culo.");const v=data.vehicle||{};setEdit(x=>({...x,vin,vehicle_year:v.model_year||x.vehicle_year,vehicle_make:v.make||x.vehicle_make,vehicle_model:v.model||x.vehicle_model,vehicle_trim:v.trim||v.series||x.vehicle_trim,vehicle_label:[v.model_year,v.make,v.model,v.trim||v.series].filter(Boolean).join(" ")||x.vehicle_label}));setNotice("VIN identificado. RevisÃ¡ los datos y guardÃ¡ la ficha.")}catch(e){setError(e?.message||"No fue posible identificar el VIN.")}finally{setVinLoading(false)}}
 // V39.9.8 FOLLOW-UP INTELLIGENCE
 async function registerProposalFollowUp(outcome){
  ...
  if((outcome==="OBJECTION"||outcome==="LOST")&&!note){
   setError(outcome==="OBJECTION"?"EscribÃ­ cuÃ¡l es la objeciÃ³n del cliente.":"IndicÃ¡ el motivo por el que se perdiÃ³ la oportunidad.");
   return;
  }
  setSaving(true);setError("");setNotice("");
  try{
   const labels={NO_RESPONSE:"Sin respuesta",INTERESTED:"Interesado",OBJECTION:"Tiene objeciÃ³n",CONTINUE:"Desea continuar",LOST:"No interesado"};
   ...
   if(outcome==="NO_RESPONSE"){
    ...
    if(previousNoResponse>=schedule.length)throw new Error("Ya se completÃ³ la cadencia DÃ­a 1, DÃ­a 3, DÃ­a 7 y DÃ­a 14. Corresponde decidir nutriciÃ³n/inactivo o cierre.");
    const day=schedule[previousNoResponse];
    nextDate=addDaysAtNine(day);
    cadenceLabel="DÃ­a "+day;
   }else if(outcome==="INTERESTED"){
    nextDate=addDaysAtNine(2);
    cadenceLabel="Interesado Â· +2 dÃ­as";
   }
   const activityPayload={
    ...
    title:"Seguimiento de propuesta Â· "+labels[outcome],
   ...
   if(outcome==="NO_RESPONSE"||outcome==="INTERESTED"){
    ...
    setNotice(labels[outcome]+" registrado. PrÃ³ximo seguimiento automÃ¡tico: "+cadenceLabel+".");
   }else{
    let target=null;
    ...
    // El validador exige OBJECION_MOTIVO antes de permitir entrar a ObjeciÃ³n / negociaciÃ³n.
    ...
    const {data:moved,error:me}=await supabase.rpc("crm_move_lead_stage_v3990",{
     p_lead_id:selected.id,p_stage_code:target,p_note:"Seguimiento: "+labels[outcome]+(note?" Â· "+note:"")
    });
    if(me)throw me;
    if(moved?.ok===false)throw new Error("No se puede avanzar. Falta: "+(moved.missing_fields||[]).join(", "));
    setNotice(outcome==="OBJECTION"
     ?"ObjeciÃ³n registrada y lead movido a ObjeciÃ³n / negociaciÃ³n."
 ...
 async function resolveCurrentObjection(continues){
  ...
  if(!response){setError("EscribÃ­ la respuesta o acuerdo alcanzado con el cliente.");return}
  setSaving(true);setError("");setNotice("");
  try{
   ...
   if(!objection)throw new Error("No encontrÃ© una objeciÃ³n pendiente para este lead.");
   ...
   await supabase.from("crm_activities").insert({
    ...
    title:continues?"ObjeciÃ³n resuelta Â· cliente continÃºa":"ObjeciÃ³n cerrada Â· cliente no continÃºa",
   ...
   let target;
   if(continues){
    // La etapa 8 exige confirmaciÃ³n del cliente segÃºn crm_validate_stage_v3990.
    const {error:ce}=await supabase.from("crm_leads").update({client_confirmed:true,updated_by:userId||null}).eq("id",selected.id);
    if(ce)throw ce;
   ...
   const {data:moved,error:me}=await supabase.rpc("crm_move_lead_stage_v3990",{
    ...
    p_note:(continues?"ObjeciÃ³n resuelta; cliente desea continuar. ":"ObjeciÃ³n no resuelta; oportunidad cerrada. ")+response
   });
   if(me)throw me;
   if(moved?.ok===false)throw new Error("No se puede avanzar. Falta: "+(moved.missing_fields||[]).join(", "));

   setNotice(continues?"ObjeciÃ³n resuelta. Lead movido a Aceptado / coordinar.":"ObjeciÃ³n cerrada. Lead movido a Venta perdida.");
   await load();
   const {data:fresh,error:fe}=await supabase.from("crm_leads").select("*").eq("id",selected.id).single();
   if(fe)throw fe;
   if(fresh)await openLead(fresh);
  }catch(e){setError(e?.message||"No fue posible resolver la objeciÃ³n.");}
  finally{setSaving(false)}
 }
 // V39.9.10 OPERATIONS HANDOFF
  // V39.9.11 CRM â†” OPERATIONS BRIDGE
  async function deliverToOperations(){
   ...
   if(!handoff.operations_assigned_to){setError("SeleccionÃ¡ el responsable operativo.");return}
   if(!handoff.documents_confirmed){setError("ConfirmÃ¡ que los documentos disponibles fueron revisados para el expediente.");return}
   if(!agreements){setError("RegistrÃ¡ los acuerdos especiales o indicÃ¡ â€œSin acuerdos especialesâ€.");return}
   setSaving(true);setError("");setNotice("");
   try{
    const balance=handoff.balance_gtq===""?null:Number(handoff.balance_gtq);
    if(balance!==null&&!Number.isFinite(balance))throw new Error("El saldo pendiente no es vÃ¡lido.");
    ...
    setNotice("Expediente entregado Â· GestiÃ³n "+(data?.management_code||"creada")+" Â· esperando embarque.");
    await load();
    const {data:fresh,error:fe}=await supabase.from("crm_leads").select("*").eq("id",selected.id).single();
...
// V39.9.12 FREIGHT PAYMENT
 ...
 async function confirmFreightPayment(){
  if(!selected||selected.stage_code!=="FREIGHT_PENDING")return;const receipt=String(freightPayment.receipt_reference||"").trim();if(!freightPayment.paid_at){setError("IndicÃ¡ la fecha de pago del flete.");return}if(!receipt){setError("IngresÃ¡ la referencia o comprobante del pago.");return}setSaving(true);setError("");setNotice("");
  try{const {data,error:e}=await supabase.rpc("crm_confirm_freight_payment_v39912",{p_lead_id:selected.id,p_paid_at:freightPayment.paid_at,p_receipt_reference:receipt,p_notes:String(freightPayment.notes||"").trim()||null});if(e)throw e;if(data?.ok===false)throw new Error("No fue posible confirmar el pago del flete.");setNotice("Pago de flete confirmado. Lead cerrado como Logrado con Ã©xito.");await load();const {data:fresh,error:fe}=await supabase.from("crm_leads").select("*").eq("id",selected.id).single();if(fe)throw fe;if(fresh)await openLead(fresh);}
 ...
 return <section className="crm-pro-shell"><header className="crm-pro-hero"><div><span className="crm-eyebrow">E&R CRM PRO · V39.9.13.1</span><h1>CRM Comercial</h1><p>Inteligencia comercial, seguimiento y cierre conectados con la operación real.</p></div><div className="crm-hero-actions"><button className="crm-secondary" onClick={load}>↻ Actualizar</button><button className="crm-primary" onClick={()=>setShowNew(true)}>＋ Nuevo lead</button></div></header>
 ...
 <nav className="crm-tabs"><button className={tab==="dashboard"?"active":""} onClick={()=>setTab("dashboard")}>📊 Dashboard</button><button className={tab==="pipeline"?"active":""} onClick={()=>setTab("pipeline")}>🧲 Pipeline</button><button className={tab==="today"?"active":""} onClick={()=>setTab("today")}>✅ Mi trabajo hoy</button><button className={tab==="contacts"?"active":""} onClick={()=>setTab("contacts")}>👥 Contactos</button></nav>{error&&<div className="crm-message error">⚠ {error}</div>}{notice&&<div className="crm-message ok">✓ {notice}</div>}
 ...
 <section className="crm-detail-section"><h3>ðŸš™ VehÃ­culo y VIN</h3><div className="crm-vin-row"><label>VIN<input value={edit.vin} maxLength={17} onChange={e=>setEdit({...edit,vin:e.target.value.toUpperCase()})}/></label><button className="crm-secondary" type="button" onClick={identifyVin} disabled={vinLoading}>{vinLoading?"Identificandoâ€¦":"ðŸ”Ž Identificar VIN"}</button></div><div className="crm-form-grid four"><label>AÃ±o<input value={edit.vehicle_year||""} onChange={e=>setEdit({...edit,vehicle_year:e.target.value})}/></label><label>Marca<input value={edit.vehicle_make||""} onChange={e=>setEdit({...edit,vehicle_make:e.target.value})}/></label><label>Modelo<input value={edit.vehicle_model||""} onChange={e=>setEdit({...edit,vehicle_model:e.target.value})}/></label><label>VersiÃ³n<input value={edit.vehicle_trim||""} onChange={e=>setEdit({...edit,vehicle_trim:e.target.value})}/></label></div><label className="crm-check"><input type="checkbox" checked={!!edit.vehicle_validated} onChange={e=>setEdit({...edit,vehicle_validated:e.target.checked})}/> VehÃ­culo revisado y validado por E&R</label></section>
 <section className="crm-detail-section"><h3>ðŸ“ LogÃ­stica</h3><div className="crm-form-grid"><label>UbicaciÃ³n del vehÃ­culo<input value={edit.vehicle_location||""} onChange={e=>setEdit({...edit,vehicle_location:e.target.value})}/></label><label>Lugar de entrega<input value={edit.delivery_location||""} onChange={e=>setEdit({...edit,delivery_location:e.target.value})}/></label><label>Subasta / proveedor<input value={edit.auction_name||""} onChange={e=>setEdit({...edit,auction_name:e.target.value})}/></label><label>Servicio solicitado<input value={edit.service_requested||""} onChange={e=>setEdit({...edit,service_requested:e.target.value})}/></label></div><label className="crm-check"><input type="checkbox" checked={!!edit.requires_tow} onChange={e=>setEdit({...edit,requires_tow:e.target.checked})}/> Requiere grÃºa / tow</label>{edit.requires_tow&&<label>Detalle de grÃºa<textarea value={edit.tow_details||""} onChange={e=>setEdit({...edit,tow_details:e.target.value})}/></label>}<div className={`crm-logistics-status ${String(edit.vehicle_location||"").trim()&&String(edit.delivery_location||"").trim()&&String(edit.auction_name||"").trim()&&String(edit.service_requested||"").trim()&&(!edit.requires_tow||String(edit.tow_details||"").trim())?"ready":"pending"}`}><strong>{String(edit.vehicle_location||"").trim()&&String(edit.delivery_location||"").trim()&&String(edit.auction_name||"").trim()&&String(edit.service_requested||"").trim()&&(!edit.requires_tow||String(edit.tow_details||"").trim())?"âœ“ LogÃ­stica completa":"âš  LogÃ­stica pendiente"}</strong><small>Se valida automÃ¡ticamente al guardar segÃºn ubicaciÃ³n, entrega, proveedor, servicio y grÃºa cuando aplique.</small></div></section>
 {selected.stage_code==="PROPOSAL_SENT"&&<section className="crm-detail-section crm-followup-pro"><div className="crm-followup-head"><div><h3>ðŸ“ž Seguimiento de propuesta</h3><p>RegistrÃ¡ quÃ© respondiÃ³ el cliente; el CRM programarÃ¡ automÃ¡ticamente el siguiente paso.</p></div><span>FOLLOW-UP</span></div><div className="crm-auto-followup"><strong>ðŸ§  ProgramaciÃ³n automÃ¡tica</strong><span>{automaticFollowUpText}</span><small>Cadencia sin respuesta segÃºn EspecificaciÃ³n CRM EyR V1: DÃ­a 1 Â· DÃ­a 3 Â· DÃ­a 7 Â· DÃ­a 14.</small></div><label>Nota del seguimiento<textarea placeholder="Ej. Cliente revisarÃ¡ la propuesta con su esposa / solicita ajuste / llamar maÃ±ana..." value={followUp.note} onChange={e=>setFollowUp({...followUp,note:e.target.value})}/></label><div className="crm-followup-actions"><button type="button" disabled={saving} onClick={()=>registerProposalFollowUp("NO_RESPONSE")}>ðŸ“µ Sin respuesta</button><button type="button" disabled={saving} onClick={()=>registerProposalFollowUp("INTERESTED")}>ðŸ‘€ Interesado</button><button type="button" disabled={saving} onClick={()=>registerProposalFollowUp("OBJECTION")}>ðŸ’¬ Tiene objeciÃ³n</button><button type="button" disabled={saving} onClick={()=>registerProposalFollowUp("CONTINUE")} className="positive">ðŸ¤ Desea continuar</button><button type="button" disabled={saving} onClick={()=>registerProposalFollowUp("LOST")} className="negative">âœ• No interesado</button></div><small className="crm-followup-hint">Sin respuesta e Interesado mantienen la propuesta activa. ObjeciÃ³n, Continuar y No interesado avanzan el pipeline automÃ¡ticamente.</small></section>}
 {selected.stage_code==="NEGOTIATION"&&<section className="crm-detail-section crm-objection-manager"><div className="crm-followup-head"><div><h3>ðŸ’¬ GestiÃ³n de objeciÃ³n</h3><p>RegistrÃ¡ cÃ³mo se atendiÃ³ la objeciÃ³n. El CRM cerrarÃ¡ la tarea y moverÃ¡ el pipeline segÃºn el resultado.</p></div><span>NEGOCIACIÃ“N</span></div><label>Respuesta / acuerdo con el cliente<textarea placeholder="Ej. Se explicÃ³ el detalle del flete y el cliente aceptÃ³ continuar..." value={objectionReply} onChange={e=>setObjectionReply(e.target.value)}/></label><div className="crm-objection-actions"><button type="button" disabled={saving} onClick={()=>resolveCurrentObjection(true)} className="positive">ðŸ¤ ObjeciÃ³n resuelta Â· continÃºa</button><button type="button" disabled={saving} onClick={()=>resolveCurrentObjection(false)} className="negative">âœ• No se logrÃ³ resolver</button></div><small className="crm-followup-hint">Si continÃºa, el sistema registra la confirmaciÃ³n del cliente y pasa a Aceptado / coordinar. Si no continÃºa, cierra la oportunidad como Venta perdida.</small></section>}
{Number(stages.find(x=>x.code===selected.stage_code)?.position)===8&&<section className="crm-detail-section crm-handoff-pro"><div className="crm-followup-head"><div><h3>ðŸ“¦ Expediente para Operaciones</h3><p>El CRM valida la evidencia ya disponible y prepara el handoff operativo.</p></div><span>HANDOFF</span></div><div className="crm-handoff-checks"><span className={selected.contact_id?"ok":""}>âœ“ Cliente</span><span className={(selected.vin||selected.vehicle_label)?"ok":""}>âœ“ VehÃ­culo</span><span className={selected.logistics_complete?"ok":""}>âœ“ LogÃ­stica</span><span className={(selected.quote_id||selected.quote_code)?"ok":""}>âœ“ CotizaciÃ³n</span><span className={selected.client_confirmed?"ok":""}>âœ“ Servicio aceptado</span></div><div className="crm-handoff-grid"><label>Estado de pago<select value={handoff.payment_status} onChange={e=>setHandoff({...handoff,payment_status:e.target.value})}><option value="PENDING">Pendiente</option><option value="PARTIAL">Pago parcial</option><option value="PAID">Pagado</option></select></label><label>Saldo pendiente (Q)<input type="number" min="0" step="0.01" value={handoff.balance_gtq} onChange={e=>setHandoff({...handoff,balance_gtq:e.target.value})} placeholder="0.00"/></label><label className="wide">Responsable operativo<select value={handoff.operations_assigned_to} onChange={e=>setHandoff({...handoff,operations_assigned_to:e.target.value})}><option value="">Seleccionar responsableâ€¦</option>{opsUsers.map(p=><option key={p.id} value={p.id}>{(p.full_name||p.email||p.id)+(p.job_title?" Â· "+p.job_title:"")}</option>)}</select></label></div><label className="crm-doc-confirm"><input type="checkbox" checked={handoff.documents_confirmed} onChange={e=>setHandoff({...handoff,documents_confirmed:e.target.checked})}/><span><strong>ðŸ“„ Documentos revisados</strong><small>Confirmo que los documentos disponibles fueron revisados antes de entregar el expediente.</small></span></label><label>Acuerdos / instrucciones para Operaciones<textarea value={handoff.agreements} onChange={e=>setHandoff({...handoff,agreements:e.target.value})} placeholder="Sin acuerdos especiales / instrucciones especÃ­ficas del cliente..."/></label><button type="button" className="crm-handoff-submit" disabled={saving} onClick={deliverToOperations}>ðŸš€ Entregar expediente a Operaciones</button><small className="crm-followup-hint">Al entregar, se crea la GestiÃ³n de ImportaciÃ³n y el CRM queda esperando a Operaciones. Cuando la gestiÃ³n pase a EMBARCADO, el CRM abrirÃ¡ automÃ¡ticamente Flete pendiente.</small></section>}
{selected.stage_code==="FREIGHT_PENDING"&&<section className="crm-detail-section crm-freight-payment"><div className="crm-followup-head"><div><h3>ðŸ’° Cobro de flete</h3><p>RegistrÃ¡ la solicitud y la evidencia real del pago antes de cerrar la venta.</p></div><span>FLETE</span></div><div className="crm-freight-summary"><strong>{freightPayment.amount_usd!=null?"$"+Number(freightPayment.amount_usd).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2}):"Monto segÃºn GestiÃ³n"}</strong><span className={freightPayment.status==="CONFIRMED"?"ok":""}>{freightPayment.status==="CONFIRMED"?"Pago confirmado":freightPayment.status==="REQUESTED"?"Cobro solicitado":"Pendiente de cobro"}</span></div>{freightPayment.requested_at&&<small className="crm-freight-date">Solicitud: {new Date(freightPayment.requested_at).toLocaleString("es-GT")}</small>}<div className="crm-form-grid"><label>Fecha de pago<input type="date" value={freightPayment.paid_at||""} onChange={e=>setFreightPayment({...freightPayment,paid_at:e.target.value})}/></label><label>Referencia / comprobante<input value={freightPayment.receipt_reference||""} onChange={e=>setFreightPayment({...freightPayment,receipt_reference:e.target.value})} placeholder="Transferencia, depÃ³sito, recibo..."/></label></div><label>Observaciones<textarea value={freightPayment.notes||""} onChange={e=>setFreightPayment({...freightPayment,notes:e.target.value})} placeholder="Detalle del cobro o pago..."/></label><div className="crm-objection-actions"><button type="button" disabled={saving||freightPayment.status==="CONFIRMED"} onClick={requestFreightPayment}>ðŸ“¨ Registrar cobro solicitado</button><button type="button" className="positive" disabled={saving||freightPayment.status==="CONFIRMED"} onClick={confirmFreightPayment}>âœ… Confirmar pago del flete</button></div><small className="crm-followup-hint">Confirmar el pago guarda fecha + comprobante, cierra Cobrar flete y mueve automÃ¡ticamente el lead a Logrado con Ã©xito.</small></section>}
<section className="crm-detail-section"><h3>ðŸ§¾ CotizaciÃ³n</h3>{selected.quote_id?<div className="crm-quote-linked">
<strong>âœ“ CotizaciÃ³n vinculada</strong>
<span>{selected.quote_code||"CotizaciÃ³n comercial"}</span>
<div className="crm-linked-quote-actions">
<button type="button" className="crm-secondary" onClick={()=>{const lead={...selected,...edit};setSelected(null);onOpenLinkedQuote?.(lead)}}>ðŸ‘ Ver cotizaciÃ³n</button>
<button type="button" className="crm-primary" onClick={()=>{const lead={...selected,...edit};setSelected(null);onSendLinkedQuoteWhatsApp?.(lead)}}>ðŸ’¬ Enviar por WhatsApp</button>
</div>
</div>:<><p className="crm-section-help">PreparÃ¡ la cotizaciÃ³n con el motor actual de E&R o vinculÃ¡ una cotizaciÃ³n existente del mismo VIN.</p><button type="button" className="crm-primary crm-full-action" onClick={()=>{const lead={...selected,...edit};setSelected(null);onOpenQuote?.(lead)}}>ï¼‹ Abrir cotizador con este VIN</button>{leadQuotes.length>0&&<div className="crm-quote-options">{leadQuotes.map(q=><button type="button" key={q.id} onClick={async()=>{setSaving(true);setError("");try{const {error:e}=await supabase.from("crm_leads").update({quote_id:q.id,quote_code:q.quote_code,updated_by:userId||null}).eq("id",selected.id);if(e)throw e;setNotice(`CotizaciÃ³n ${q.quote_code} vinculada al lead.`);const fresh={...selected,quote_id:q.id,quote_code:q.quote_code};setSelected(fresh);await load()}catch(e){setError(e?.message||"No fue posible vincular la cotizaciÃ³n.")}finally{setSaving(false)}}}><span>{q.quote_code}</span><small>{q.status||"DRAFT"} Â· {q.vehicle_label||q.vin}</small></button>)}</div>}</>}</section>
 <section className="crm-detail-section"><h3>ðŸ¤ Acuerdos comerciales</h3><label className="crm-check"><input type="checkbox" checked={!!edit.client_confirmed} onChange={e=>setEdit({...edit,client_confirmed:e.target.checked})}/> Cliente confirmÃ³ que desea continuar</label><label>Acuerdos / observaciones<textarea value={edit.special_agreements||""} onChange={e=>setEdit({...edit,special_agreements:e.target.value})} placeholder="Condiciones especiales, compromisos, observacionesâ€¦"/></label></section>
 <section className="crm-detail-section"><h3>âœ… Tareas</h3><div className="crm-mini-list">{detailTasks.map(t=><div key={t.id}><span>{OPEN.has(t.status)?"â—":"âœ“"}</span><div><strong>{taskTitle(t)}</strong><small>{t.status} Â· {dueLabel(t.due_at)}</small></div></div>)}{!detailTasks.length&&<small>Sin tareas registradas.</small>}</div></section>
 <section className="crm-detail-section"><h3>ðŸ•˜ Historial</h3><div className="crm-mini-list">{activities.map(a=><div key={a.id}><span>â€¢</span><div><strong>{a.title||a.activity_type}</strong><small>{a.description||""} {a.created_at?`Â· ${new Date(a.created_at).toLocaleString("es-GT")}`:""}</small></div></div>)}{!activities.length&&<small>Sin actividad registrada.</small>}</div></section></div>}<footer className="crm-drawer-footer"><button className="crm-secondary" onClick={()=>setSelected(null)}>Cerrar</button><button className="crm-primary" onClick={saveLead} disabled={saving}>{saving?"Guardandoâ€¦":"Guardar ficha"}</button></footer></aside></div>}
 {showNew&&<div className="crm-modal-backdrop" onMouseDown={()=>!saving&&setShowNew(false)}><form className="crm-modal" onSubmit={createLead} onMouseDown={e=>e.stopPropagation()}><header><div><span className="crm-eyebrow">NUEVA OPORTUNIDAD</span><h2>Crear lead</h2></div><button type="button" onClick={()=>setShowNew(false)}>Ã—</button></header><label>Nombre del contacto *<input required value={form.full_name} onChange={e=>setForm({...form,full_name:e.target.value})} placeholder="Nombre completo"/></label><div className="crm-form-grid"><label>WhatsApp / telÃ©fono<input value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} placeholder="+502 ..."/></label><label>Correo<input type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/></label><label>VIN<input value={form.vin} onChange={e=>setForm({...form,vin:e.target.value.toUpperCase()})} maxLength={17}/></label><label>VehÃ­culo<input value={form.vehicle_label} onChange={e=>setForm({...form,vehicle_label:e.target.value})} placeholder="2020 Toyota Tacoma"/></label></div><label>Prioridad<select value={form.priority} onChange={e=>setForm({...form,priority:e.target.value})}><option value="NORMAL">Normal</option><option value="HIGH">Alta</option><option value="URGENT">Urgente</option><option value="LOW">Baja</option></select></label><footer><button type="button" className="crm-secondary" onClick={()=>setShowNew(false)}>Cancelar</button><button className="crm-primary" disabled={saving}>{saving?"Creandoâ€¦":"Crear lead"}</button></footer></form></div>}
 </section>
}
