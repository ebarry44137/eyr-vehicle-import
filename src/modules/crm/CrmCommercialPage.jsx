...
function CrmCommercialPage({supabase,userId,userName="Usuario E&R",onOpenQuote,onOpenLinkedQuote,onSendLinkedQuoteWhatsApp,refreshSignal=0}){
 ...
 const load=useCallback(async()=>{setLoading(true);setError("");try{const [s,l,t,c]=await Promise.all([supabase.from("crm_pipeline_stages").select("*").eq("active",true).order("position"),supabase.from("crm_leads").select("*").order("created_at",{ascending:false}),supabase.from("crm_tasks").select("*").order("due_at",{ascending:true,nullsFirst:false}),supabase.from("crm_contacts").select("*").order("created_at",{ascending:false})]);const e=[s.error,l.error,t.error,c.error].find(Boolean);if(e)throw e;setStages(s.data||[]);setLeads(l.data||[]);setTasks(t.data||[]);setContacts(c.data||[])}catch(e){setError(e?.message||"No fue posible cargar CRM Comercial.")}finally{setLoading(false)}},[supabase]);
 useEffect(()=>{load()},[load]);
 ...
 const metrics=useMemo(()=>({open:leads.filter(x=>!["WON","LOST"].includes(x.stage_code)).length,quoting:leads.filter(x=>["QUOTING","PROPOSAL_SENT","NEGOTIATION"].includes(x.stage_code)).length,won:leads.filter(x=>x.stage_code==="WON").length,overdue:tasks.filter(x=>OPEN.has(x.status)&&x.due_at&&new Date(x.due_at)<new Date()).length}),[leads,tasks]);
