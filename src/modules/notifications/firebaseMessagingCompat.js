...
async function getFirebase() {
  if (typeof window === "undefined") {
    throw new Error("Firebase Messaging solo está disponible en el navegador.");
  }
...
export async function listenForegroundMessages(onMessage) {
  ...
  foregroundUnsubscribe = messaging.onMessage(async (payload) => {
    try {
      ...
      if (Notification.permission === "granted" && title) {
        ...
        await registration.showNotification(title, {
          body: body || "Tenés una nueva notificación.",
          icon: "/branding/icon-192.png",
          badge: "/branding/icon-192.png",
          tag: payload?.data?.tag || "eyr-fcm",
          data: { url },
        });
      }
    } catch (error) {
      console.error("FCM FOREGROUND MESSAGE ERROR:", error);
    }
  });
...
export async function getFirebasePushStatus({ supabase } = {}) {
  ...
  if (!supabase) {
    return {
      state: permission === "granted" ? "PERMISSION_ONLY" : permission.toUpperCase(),
      permission,
      registered: false,
    };
  }
  ...
  try {
    ...
    const registered = devices.some((row) => row?.active === true);
    ...
    console.warn("FCM STATUS CHECK ERROR:", error?.message);
    return {
      state: permission === "granted" ? "PERMISSION_ONLY" : permission.toUpperCase(),
      permission,
      registered: false,
      error: error?.message || "No fue posible validar el dispositivo.",
    };
  }
}
